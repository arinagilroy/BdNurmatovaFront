...
function newTbodyTr(obj, index) {
	var elem = `
		<tr data-toggle="modal" data-target="#myModal" onclick="showTr(event, ${index}, ${obj.id})">
			...
			<td>${ obj.phone }</td>
			<td>
				<button type="button" class="btn btn-danger" data-toggle="kss" onclick="deleteThisProduct(${ obj.id })">Delete</button>
			</td>
		</tr>`;
...
class PostApi {
  static getProducts() {
    return fetch(base_url+'/manufactCompany', {
      method: 'GET'
    }).then(res => res.json());
  }

  static deleteProduct(id) {
		return fetch(base_url+'/manufactCompany?id='+id, {
			method: 'DELETE'
		}).then(res => res.json());
	}
